...
def validate_email_domain_literal(domain_literal, allow_domain_literal=False):
    ...
    if re.match(r"^[0-9\.]+$", domain_literal):
        try:
            addr = ipaddress.ip_address(domain_literal)
        except ValueError:
            raise EmailSyntaxError("The IPv4 address in brackets after the @-sign is not valid.")
        ...
        # Otherwise fall through and parse another way.

    # If it begins with "IPv6:" it's an IPv6 address.
    if domain_literal.startswith("IPv6:"):
        try:
            addr = ipaddress.ip_address(domain_literal[5:])
        except ValueError:
            raise EmailSyntaxError("The IPv6 address in brackets after the @-sign is not valid.")
